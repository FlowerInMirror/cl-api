...
(function ($) {
    $.tagator = function (element, options) {
        ...
        plugin.init = function () {
            ...
            $(box_element).addClass('tagator options-hidden fl mr10');
            $(box_element).css({
            ...
            $(input_element).bind('keyup', function (e) {
                var strVal = $(input_element).val();
                var str = strVal.substr(strVal.length - 1);
                if (str == "；" || str == "，") {
                    $(input_element).val(strVal.substr(0, strVal.length - 1));
                    e.preventDefault();
                    if (selected_index === -1) {
                        if ($(input_element).val() !== '') {
                            addTag($(input_element).val());
        ...
        var refreshTags = function () {
            $(tags_element).empty();
            var tags = $(element).val().replace("，", ",").split(',');
