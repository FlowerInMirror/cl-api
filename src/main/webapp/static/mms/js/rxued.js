...
rxued.table = {
    ...
    //tb1：表头表格，tb2：数据表格，chaObj:需要减去的高度添加该class，resultObj：数据表格外面的div
    countTB: function (tb1, tb2, chaObj,otherheight, resultObj, fn) {
        ...
        var chaHeight = 0;
        for (var i = 0; i < chaObj.length; i++)
        {
            chaHeight += chaObj.eq(i).outerHeight(true);
        }

        var aHeight = 0;
        var ReductionBox = $(".j_outerHeight"); //所有需要减去高度的元素加上CLASS："j_outerHeight"
        for (var i = 0; i < ReductionBox.length; i++) {
            aHeight += ReductionBox.eq(i).outerHeight(true);
        }
        var ResultHeight = $(window).height() - parseInt(aHeight) - chaHeight - otherheight;
        resultObj.css("height", ResultHeight + 'px');
...
//              }
    ...
    enGroupLarge: function (clickelement, alertdivid, curid) {
       ...
       // });
        //点击下一张按钮
        $(document).off("click", "#" + alertdivid + " .previewNext");
        ...
        //点击上一张按钮
        $(document).off("click", "#" + alertdivid + " .previewPrev");
...
rxued.init = {
    countH: function (maindiv, otherheight, fn) {
        ...
        {
            aheight += $(".j_outerHeight").eq(j).outerHeight(true);
        }
        maindiv.css("height", owindowH - aheight - otherheight + "px");
    },
    countMaxH: function (maindiv, otherheight, fn) {
        var owindowH = $(window).height();
        var aheight = 0;
        for (var j = 0; j < $(".j_outerHeight").length; j++) {
            aheight += $(".j_outerHeight").eq(j).outerHeight(true)
        }
        maindiv.css("max-height", owindowH - aheight - otherheight + "px");
        if (typeof (fn) == "function") {
            fn()
        }
    }
